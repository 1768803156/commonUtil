...
public class ActivityManager {
    ...
    public synchronized boolean isActivityRunning(Class<?> c) {
        for (Activity ac : createdActivities) {
            if (ac.getClass().getSimpleName().equals(c.getSimpleName())) return true;
        }
        return false;
    }

    public synchronized Class<?> getCurrentStackTop() {
        if (createdActivities.size() == 0) return null;
    ...
    public synchronized void finsihOtherActivity(String activityName) {
        int size = createdActivities.size();
        for (int i = 0; i < size; i++) {
            if (!createdActivities.get(i).getClass().getSimpleName().equals(activityName)) {
                createdActivities.get(i).finish();
    ...
    public synchronized void clearTopActivity(String activityName) {
        ...
        boolean onTop = false;
        for (int i = 0; i < size; i++) {
            if (onTop) createdActivities.get(i).finish();
            if (createdActivities.get(i).getClass().getSimpleName().equals(activityName)) {
                onTop = true;
            }
        }
    ...
    public synchronized void clearTopFinishSelfActivity(String activityName) {
        int size = createdActivities.size();
        boolean onTop = false;
        for (int i = 0; i < size; i++) {
            if (createdActivities.get(i).getClass().getSimpleName().equals(activityName)) {
