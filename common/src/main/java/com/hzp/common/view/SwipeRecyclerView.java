...
import com.hzp.common.view.footerView.BaseFooterView;
import com.hzp.common.view.footerView.SimpleFooterView;

/**
 * @author HZP
 * @version created at 2017/3/23 2:27
 */
public class SwipeRecyclerView extends FrameLayout implements SwipeRefreshLayout.OnRefreshListener {

    public final static long REFRESHTIME = 500;
    public final static long LOADMORETIME = 100;
    ...
    private void setupSwipeRecyclerView() {
        ...
        mRefreshLayout.setColorSchemeColors(ContextCompat.getColor(getContext(), android.R.color.holo_blue_bright), ContextCompat.getColor(getContext(), android.R.color
                .holo_green_light), ContextCompat
                .getColor(getContext(), android.R.color.holo_orange_light), ContextCompat.getColor(getContext(), android.R.color.holo_red_light));
        mRefreshLayout.setFadingEdgeLength(0);
        ...
        recyclerView.addOnScrollListener(new OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                if (!isLoadMoreEnable || isRefreshing() || isLoadingMore || !isHasLoadMore) {
                    return;
                }
     ...
     * @param emptyView the view to be showing when the data set size is zero
     */
    public void setEmptyView(View emptyView, String errorStr) {
        if (mEmptyView != null) {
            removeView(mEmptyView);
        }
        this.mEmptyView = emptyView;
        ...
        TextView errorTV = emptyView.findViewById(R.id.errorTV);
     ...
     * @param emptyView the view to be showing when the data set size is zero
     */
    public void setEmptyViewNoClick(View emptyView) {
        if (mEmptyView != null) {
            removeView(mEmptyView);
        }
        this.mEmptyView = emptyView;
    ...
    /**
     * {@link #onNetChange(boolean isAvailable)}
    ...
    /**
     * {@link #onNoMore(CharSequence message)}
    ...
    /**
     * {@link #onError(CharSequence message)}
    ...
    private class WrapperAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
        ...
        public void onViewAttachedToWindow(RecyclerView.ViewHolder holder) {
            ...
            if (lp != null && lp instanceof StaggeredGridLayoutManager.LayoutParams && isLoadMoreItem(holder.getLayoutPosition())) {
    ...
    public void setHasLoadMoreAble(boolean isHasLoadMore) {
        this.isHasLoadMore = isHasLoadMore;
    }

    public void loadMoreType(SwipeRecyclerView mRecyclerView, int pageNum, int dataSize) {
        if (pageNum == 1) {
            if (dataSize < 10) mRecyclerView.onNoMore("");
        } else {
            if (dataSize == 0) mRecyclerView.onNoMore("");
        }
    }
