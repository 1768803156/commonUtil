...
/**
 * @author HZP
 * @version created at 2017-12-20 17:33
 */
public class MyRatingBar extends View {
    @SuppressWarnings("unused")
    private static final String TAG = "RatingBar";
    protected static final int DEFAULT_FILLED_DRAWABLE = R.mipmap.star_2icon;
    protected static final int DEFAULT_EMPTY_DRAWABLE = R.mipmap.star_0icon;
    ...
    protected void init(AttributeSet attributeSet) {
        if (attributeSet != null) {
            TypedArray a = getContext().obtainStyledAttributes(attributeSet, R.styleable.MyRatingBar);
            filledDrawable = a.getResourceId(R.styleable.MyRatingBar_filledDrawable, DEFAULT_FILLED_DRAWABLE);
            emptyDrawable = a.getResourceId(R.styleable.MyRatingBar_emptyDrawable, DEFAULT_EMPTY_DRAWABLE);
            mStarSize = a.getDimensionPixelSize(R.styleable.MyRatingBar_starSize, getPixelValueForDP(20));
            mMaxCount = a.getInt(R.styleable.MyRatingBar_numStars, 5); // you usually go 1-5 stars when rating
            mMinSelectionAllowed = a.getInt(R.styleable.MyRatingBar_minAllowedStars, 0);
            mMargin = a.getDimensionPixelSize(R.styleable.MyRatingBar_starSpacing, getPixelValueForDP(5));
            mRating = a.getFloat(R.styleable.MyRatingBar_rating, mMinSelectionAllowed);
            isIndicator = a.getBoolean(R.styleable.MyRatingBar_isIndicator, false);
            mStepSize = a.getFloat(R.styleable.MyRatingBar_stepSize, 1);
            a.recycle();
        } else {
            setDefaultDrawables();
        }
